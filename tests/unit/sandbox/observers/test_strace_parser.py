...
class TestConnectIPv6:
    ...
    def test_connect_ipv6_port_53_is_dns(self):
        ev = _ev(
            "1234 connect(4, {sa_family=AF_INET6, sin6_port=htons(53), sin6_flowinfo=0}, 28) = 0"
        )
        assert ev is not None
        assert ev.kind == "dns"
...
class TestConnectUnix:
    def test_connect_unix_socket(self):
        ev = _ev('1234 connect(3, {sa_family=AF_UNIX, sun_path="/tmp/mysock"}, 20) = 0')
        assert ev is not None
        assert ev.kind == "connect"
        assert ev.target == "unix:/tmp/mysock"

    def test_connect_unix_abstract_socket(self):
        ev = _ev('1234 connect(3, {sa_family=AF_UNIX, sun_path="@/tmp/dbus"}, 20) = 0')
        assert ev is not None
        assert ev.kind == "connect"
...
class TestOpenat:
    ...
    def test_openat_creat_is_write(self):
        ev = _ev('1234 openat(AT_FDCWD, "/tmp/install.sh", O_WRONLY|O_CREAT|O_TRUNC, 0777) = 5')
        assert ev is not None
        assert ev.kind == "write"
...
class TestObserverCanary:
    ...
    def test_canary_write_mode_still_canary(self):
        ev = _ev(f'1234 openat(AT_FDCWD, "{OBSERVER_CANARY_MARKER}", O_WRONLY|O_CREAT) = 3')
        assert ev is not None
        assert ev.kind == "observer_canary"
...
class TestExecve:
    def test_execve_kind_is_exec(self):
        ev = _ev('1234 execve("/bin/sh", ["/bin/sh", "-c", "echo hi"], /* 15 vars */) = 0')
        assert ev is not None
        assert ev.kind == "exec"

    def test_execve_target_is_path(self):
        ev = _ev('1234 execve("/usr/bin/node", ["/usr/bin/node", "index.js"], 0x7fff) = 0')
        assert ev is not None
        assert ev.target == "/usr/bin/node"

    def test_execve_argv_extracted(self):
        ev = _ev('1234 execve("/bin/sh", ["/bin/sh", "-c", "npm run build"], /* 15 vars */) = 0')
        assert ev is not None
    ...
    def test_execve_pid_preserved(self):
        ev = _ev('5678 execve("/bin/curl", ["/bin/curl", "-s", "http://evil.com"], 0x7fff) = 0')
        assert ev is not None
        assert ev.pid == 5678
...
class TestClone:
    ...
    def test_clone3_also_parsed(self):
        ev = _ev(
            "1234 clone3({flags=CLONE_CHILD_CLEARTID|SIGCHLD, exit_signal=SIGCHLD}, 88) = 9999"
        )
        assert ev is not None
        assert ev.kind == "clone"
...
class TestPidFormats:
    ...
    def test_strace_5x_large_pid(self):
        ev = _ev('[pid 65535] openat(AT_FDCWD, "/etc/passwd", O_RDONLY) = 3')
        assert ev is not None
        assert ev.pid == 65535
...
class TestNonEventLines:
    def test_signal_delivery_returns_none(self):
        ev = _ev(
            "1234 --- SIGTERM {si_signo=SIGTERM, si_code=SI_USER, si_pid=999, si_uid=1000} ---"
        )
        assert ev is None
...
class TestRedaction:
    ...
    def test_scrub_applied_to_execve_argv(self):
        ...
        ev = _ev(
            f'1234 execve("/bin/sh", ["/bin/sh", "-c", "echo {secret}"], /* 5 vars */) = 0',
            scrub=[secret],
        )
        assert ev is not None
        for arg in ev.argv:
...
class TestFixtureParseRatio:
    ...
    def test_parse_ratio_gte_0_9(self, fixture: str):
        ...
        ratio = events / total
        assert ratio >= 0.9, f"{fixture}: parse ratio {events}/{total} = {ratio:.2%} < 90%"
...
class TestPartialLineBuffering:
    def test_partial_bytes_buffered_until_newline(self):
        ...
        ev = obs.parse_event(b'1234 openat(AT_FDCWD, "/etc/passwd", O_RDONLY) = 3', [])
        assert ev is None
        # Feed newline → event
